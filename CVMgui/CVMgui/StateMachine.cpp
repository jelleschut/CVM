...
event_e StateMachine::statemachine(event_e eventIn) {
   ...
   switch(currentState) {
      ...
      case S_INIT:
         ...
         pDialog->enableCentButtons(0,5,true);
      ...
      case S_1E:
         ...
         switch(eventIn) {
            ...
            case E_MONEY_NOTENOUGH:
               ...
               break;
            default:
               pDialog->setLogger("S_1E System ERROR: Unknown event");
               break;
         }
         break;

      case S_3E:
         eventIn = checkCents(300);
         switch(eventIn) {
            case E_MONEY_UNRECOGNIZED:
               nextState = S_UNRECOGNIZED;
               eventOut = E_SEQ;
               break;
            default:
               pDialog->setLogger("S_3E System ERROR: Unknown event");
               break;
         }
         break;
      ...
      case S_UNRECOGNIZED:
         pDialog->setLogger("-state: UNRECOGNIZED, unrecognized coin returned");
         nextState = S_WAIT_FOR_COINS;
         eventOut = E_NO;
         break;
...
event_e StateMachine::checkCents(const int cents) {
   ...
   return E_MONEY_UNRECOGNIZED;
}

void StateMachine::showMoney() {
   std::stringstream displayBuffer;
   displayBuffer << " Insterted money = " << money << " cents";
